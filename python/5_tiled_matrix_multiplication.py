...
@cute.kernel
def tiled_matrix_multiplication_kernel(A: cute.Tensor, B: cute.Tensor, C: cute.Tensor):
    M, K = A.shape
    N = B.shape[1]
    ...
    base = cute.arch.get_dyn_smem(cute.Float16, alignment=128)  # base pointer (f16*)
    a_elems = bdim_y * TILE_K

    A_ptr = base  # first region
    B_ptr = base + a_elems  # second region (element offset add is supported)

    layout_a = cute.make_layout((bdim_y, TILE_K), stride=(TILE_K, 1))
    layout_b = cute.make_layout((TILE_K, bdim_x), stride=(bdim_x, 1))

    a_s = cute.make_tensor(A_ptr, layout_a)
    b_s = cute.make_tensor(B_ptr, layout_b)

    n_tiles = (K + TILE_K - 1) // TILE_K
    acc = cute.Float32(0.0)

    for k in range(n_tiles, unroll_full=True):
        kk = tx
        while kk < TILE_K:
            col_g = k * TILE_K + kk
            a_s[ty, kk] = A[ty_g, col_g] if (ty_g < M and col_g < K) else cute.Float16(0)
            kk += bdim_x

        kk = ty
        while kk < TILE_K:
            row_g = k * TILE_K + kk
            b_s[kk, tx] = B[row_g, tx_g] if (row_g < K and tx_g < N) else cute.Float16(0)
            kk += bdim_y

        cute.arch.sync_threads()

        for kk in range(TILE_K, unroll_full=True):
            acc += a_s[ty, kk].to(cute.Float32) * b_s[kk, tx].to(cute.Float32)
...
def tiled_matrix_multiplication(M_d, N_d, P_d):
    ...
    # bytes = (A_s elems + B_s elems) * sizeof(fp16)
    smem_bytes = (block_y * TILE_K + TILE_K * block_x) * 2
...
if __name__ == "__main__":
    ...
    print(
        f"[Kernel tiled] mean {tld_mean:.3f} ms  std {tld_std:.3f} ms  {gflops(H, K, W, tld_mean):.2f} GFLOP/s, TILE_K={TILE_K}"
    )
